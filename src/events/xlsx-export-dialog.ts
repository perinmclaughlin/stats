...
export class XLSXExportDialog {
  ...
  async exportRawXLSX() {
    ...
    this.downloadXLSX(this.event, xlsx);
  }
  ...
  private downloadXLSX(event: EventEntity, xlsx) {
    let name = this.fileName;
    let data = [["TEAMS"], ["TEST"]];
    let file = XLSX.utils.book_new();
    let sheet = XLSX.utils.aoa_to_sheet(data);
    XLSX.utils.book_append_sheet(file, sheet, "TEST");
    XLSX.writeFile(file, "YAY");
    console.log(XLSX.writeFile(file, "YAY"));
  }
